...
C<packs>, and C<variables>. They are explained in depth in the
following sections.
...
Most actions (with the notable exception of C<sketch>) also allow a
simpler, string-based form with the following format:
...
In the following sub-sections, only key-value pairs specific to the
respective types will be explained.
...
Every L</prepare> and L</stroke> action generates as output such an
...
using tag C<:default:>, more information in L<< /Action: C<frame> >>.
...
=head2 Custom Pack Implementations
