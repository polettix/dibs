...
L<http://github.com/polettix/dibspack-basic>), so multiple programs can
be packed together, also including possible additional ancillary files
(e.g. in
L<https://github.com/polettix/dibspack-basic/tree/master/perl/>, the
C<build> program relies upon C<carton> and C<cpanm>, saved in the same
directory, as well as the C<lib.sh> shell library stored one directory
above).
