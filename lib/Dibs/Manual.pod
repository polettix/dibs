...
C<dibs>, because they are located in some specific place:

=over

=item *

the C<packs> sub-directory of the project directory


=item *

somewhere inside the C<src> sub-directory
...
Dynamic packs, on the other hand, need to be gathered before being used.
In this case, C<dibs> first gets the pack contents and stores it in one
of two possible positions, then treats them like static ones (after they
...
when using the action (as specified in L</Configuratio actions>), like
this:
