...
powerful, but it allows you to execute either direct commands or some
...
`dibs` allows defining _packs_ of operations.footnote:[The name is inspired to
https://www.heroku.com/[Heroku]'s buildpacks, but the semantics is nowadays
very different.] in many different ways, which include grouping them in a git
repository that can be shared across multiple projects. An example of such a
repository is https://github.com/polettix/dibspack-basic[dibspack-basic].
...
To generate an image, multiple _actions_ are required:

- 
- a single operation step performed inside a container is a _stroke_. It can
  be thought as a single 'RUN` step inside a Dockerfile;
- 
...
The setup phase collects all available information for the specific run. It
merges together different sources (command line parameters, environment
variables, and the configuration file), making also sure that the source code
is fetched and made ready for operating the different steps.

The steps phase runs through a customized series of steps. Each step runs
a series of operations starting from a container image and operating in layers
on top of it; these operations are performed through _dibspacks_ (see below).
When the series is complete, the step might just through away the resulting
pile of container layers, or save them in a container image. This makes it
possible to ditch away container layers for _build_ steps, and keep those for
_bundle_ steps (where the target image is packaged).
